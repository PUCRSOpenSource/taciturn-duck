class CurriculosController < ApplicationController
	...
	# GET /curriculos/new
	def new
		puts "curriculos: #{@curriculo}"
		@curriculo = Curriculo.new
		@curriculo.disciplinas.build
		@disc = Disciplina.all
	end
	...
	def curriculo_params
		params.require(:curriculo).permit(:identificador, :curso_id, disciplinas_attributes: [:id, :_destroy])
	end
end
